...
# 시계열 데이터 그래프 애니메이션
# 전세계 국가(핀란드, 한국, 베트남)에 대한 연도별 기대수명과 국내총생산(GDP)와의 관계

# gapminder dataset 확인
gapminder

# 전세계 국가 중에서 3개국(핀란드, 한국, 베트남) 데이터만 필터링해서 추출
...
anim = ggplot(df, aes(x=gdpPercap, y=lifeExp, size=pop, color=country)) +
  ...
  labs(title="연도: {frame_times}", x="1인당 GDP", y="기대수명") +
...
bg = image_read("D:/MINJU/R_data/background.png") # 배경 이미지
target = image_read("D:/MINJU/R_data/target.png") # 과녘판 이미지
arrow = image_read("D:/MINJU/R_data/arrow.png") # 화살
...
# 초기 각도와 화살이 움직이는 속력, 중력에 대한 곡선 형태는 검색하여 임의로 설정하여 장쳔스럽게 보이도록 하세요.
