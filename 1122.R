...
df3$x[df3$x==0] = 10
df3$x[is.na(df3$x)] = 4
df3$x[is.nan(df3$y)] = 60
df3$x[is.nan(df3$z)] = 30
df3$x[df3$z>=Inf] = 10
df3$x[df3$z<=Inf] = 20
df3$x[df3$w==""] = "unknown"
...
df3$x[is.nan(df3$y)] = median(df3$y[is.finite(df3$y)])
df3$x[is.nan(df3$z)] = mean(df3$z[is.finite(df3$z)])
df3$x[is.nan(df3$z)] = mean(df3$z[is.finite(df3$z)])
df3$x[df3$z>=Inf] = max(df3$z[is.finite(df3$z)])
df3$x[df3$z<=Inf] = min(df3$z[is.finite(df3$z)])
df3$x[df3$w==""] = "다"

# 3단계 : 데이터 통합
# 여러 데이터프레임을 하나로 합치는 것

df1 = data.frame(ID=1:3, 성명=c("장발장", "파틴", "자베르"))
...
# left outer join(왼쪽 외부조인 : 왼쪽의 데이터 프레임이 손실되지 않음)
left_join(df1, df2, by='ID')

# right outer join(오른쪽 외부조인 : 오른쪽의 데이터 프레임이 손실되지 않음)
