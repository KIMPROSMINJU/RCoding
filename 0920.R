...
# 막대그래프(그래프 데이터 초기화 - 그래프 모양 - 제목, 라벨 설정)
...
ggplot(data = df1, aes(x=부서, y=매출)) +
  geom_bar(stat="identity", fill=c("pink", "tomato", "gold"), color="black") +
  labs(title="팀별 영업실적", x="부서명", y= "매출(억원)")
...
# 1분기와 2분기 데이터를 행으로 연결해서 하나의 데이터프레임으로 자징
df = rbind(df1, df2)
df
...
# 조건 3> 그래프 제목 : 부서별분기별 영업실적, x축 제목 : 부서명, y축 제목 : 영업매출(억원)
...
ggplot(df, aes(x=부서, y=매출, fill=분기)) +
  geom_bar(stat="identity") +
  scale_fill_manual(values=c("pink", "magenta", "purple")) +
  labs(title="부서별분기별 영업실적", x="부서명", y= "영업매출(억원)")
...
ggsave("D:/MINJU/R_data/Temp/Graph1.png", width=10, height=13, units="cm")
...
ggplot(data=df, aes(x=월, y=매출, group=부서, group=부서)) +
...
# 점의 모양, 색상, 크기 설정
ggplot(data=women, aes(x=height, y=weight)) +
  geom_point(shape=11, color="tomato", size=2) +
  labs(title="미국 30~39세 여성의 키와 몸무게의 상관관계", x="키(inch)", y="몸무게(pound)")
